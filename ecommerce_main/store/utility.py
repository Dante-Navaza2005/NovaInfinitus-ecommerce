...
def dynamic_upload_path(instance, filename):
    # Sanitize the name attribute to create a safe folder name
    if instance.name:
        safe_name = re.sub(r'[^a-zA-Z0-9_-]', '_', instance.name.lower())
    else:
        safe_name = "unknown"  # Fallback if the name is None
    # Create the directory path using the model name and the sanitized name attribute
    model_name = instance.__class__.__name__.lower()
    return os.path.join(model_name, safe_name, filename)
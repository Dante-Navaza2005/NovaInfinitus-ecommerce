...
from .utility import dynamic_upload_path
...
class Product(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['name', 'category', 'product_type'], name='unique_product')
        ]
...
class Color(models.Model):
    name = models.CharField(max_length=200, null=True, blank=True, unique=True)
    code = models.CharField(max_length=200, null=True, blank=True, unique=True) #? color code is a hex code

    def __str__(self) :
        return str(self.name)

class ItemStock(models.Model):
    product = models.ForeignKey(Product, blank=True, null=True, on_delete=models.SET_NULL)  # One itemstock is associated to a single product, but a product can have many itemstocks
    ...
    def __str__(self):
        ...
        size = self.size if self.size else "No Size"
        return f"{product_name} - Color: {color_name} - Size: {size}"

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["product", "color", "size"], name="unique_itemstock"
            )
        ]
...
class Banner(models.Model) :
    ...
    def __str__(self):
        link_output = self.link_output if self.link_output else "No link output"
        return f"{link_output} - Active: {self.active}"
